...
export const MachinerysList = ({ machinerys, visibleMacinerys, checkedMarker }) => {
  const filteredMachinerys = machinerys.filter(machinery => {
    return visibleMacinerys.some(visible => visible._id === machinery._id);
  });
  const checkedMachinerys = machinerys.filter(machinery => machinery._id === checkedMarker._id);
  ...
  return (
    <>
      ...
      {filteredMachinerys.length === 0 && <p>There are no machine in this part of the map!</p>}
      {checkedMachinerys.length === 0 && <ul className="machinery-list">
